<template>
  <v-row justify="center">
    <v-col sm="12">
      ...
      <v-btn
        block
        class="color-github text-capitalize mb-3"
        @click="githubLogin"
        color="indigo lighten-4"
      >
        <v-icon left class="color-github__icon" size="22">
          mdi-github
        </v-icon>
        Githubアカウントでログイン
      </v-btn>
...
/*
下記サイトを参考。ただし、ボタンの色をそろえたかったのでカスタマイズしている
https://github.com/joey-i/nuxt-firebase-auth-sample/blob/master/components/SocialLogin.vue
*/
...
.color-google {
  @include social_button(#fff, #212121);
  @at-root {
    #{&}__icon > svg {
      position: absolute;
    }
  ...
  methods: {
    googleLogin() {
      ...
      this.$fire.auth
        ...
        .then(authData => {
          this.registUser(authData);
        })
        .catch(err => {
          this.$parent.socialLoginErrorMsg = "ログインに失敗しました";
        });
    },
    githubLogin() {
    ...
    registUser(authData) {
      const usersRef = this.$fire.database.ref("users/" + authData.user.uid);
      usersRef.once("value").then(snapshot => {
        // 新規登録時はRealtime Databaseにも登録
        if (snapshot.val() === null) {
          this.$fire.database.ref("users/" + authData.user.uid).set({
            email: authData.user.email,
            name: authData.user.displayName
          });
          this.$router.push("/account/mypage"); // 新規登録時はマイページへ遷移
        } else {
          this.$router.push("/private"); // ログイン時は非公開ページへ遷移（"/"にすればトップへ）
        }
      });
    }
  }
